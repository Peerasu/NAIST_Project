...
def make_fold_WSI(args):
    ...
    if "5Fold_Test_WSI.csv" in file_name:
        ...
        # Name_Orial_ = df.values[:, 0]
        Name_Orial_ = ['_'.join(name.split('_')[:-1]) for name in df.values[:, 0]]
        ...
        train_df.to_csv(args.Train_LogData_WSI.format(str(args.num_fold)), index=False)
        test_df = df[df.fold == 0].reset_index(drop=True)

        Index = np.arange(len(test_df))
        np.random.shuffle(Index)
        test_df = test_df.iloc[Index].reset_index(drop=True)
        test_df.to_csv(args.Test_LogData_WSI.format(str(args.num_fold)), index=False)
...
def make_fold_Patient(args):
    ...
    if "5Fold_Test_Patient.csv" in file_name:
        ...
        train_df.to_csv(args.Train_LogData_Patient.format(str(args.num_fold)), index=False)
        test_df = df[df.fold == 0].reset_index(drop=True)

        Index = np.arange(len(test_df))
        np.random.shuffle(Index)
        test_df = test_df.iloc[Index].reset_index(drop=True)
        test_df.to_csv(args.Test_LogData_Patient.format(str(args.num_fold)), index=False)
...
# my function
def Image_Show_Aug(image, image1, image2):
    images = [image, image1, image2]
    labels = ['Original', 'Aug_A', 'Aug_B']
...
class HubmapDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Image_Show_Aug(image, image1, image2)
