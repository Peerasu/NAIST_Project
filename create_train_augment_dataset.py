...
original_dataset_dir = '../../../../../mnt/d/peerasu/Tile_train_4'

# Path to the new dataset directory where augmented images will be saved
new_dataset_dir = '../../../../../mnt/d/peerasu/Tile_train_4_augment'

# Path to the true train label CSV file
original_train_label = "../annotation/true_train_label_4.csv"

# Path to save the CSV file
csv_file_path = "../annotation/true_train_label_4_augment.csv"
...
for filename in os.listdir(original_dataset_dir):
    ...
    label = true_train_labels.get((filename.split('.'))[0])
    
    # Append data to CSV list
    csv_data.append({'Sample_Name': (filename.split('.'))[0], 'Label': label})
    
    # Apply augmentation twice to generate two augmented images
    augmented_images = []
    for _ in range(4):
